...
containsUnsigsNFT v tn = valueOf v (policyUnsigs contractInfo) (TokenName ((prefixUnsigs contractInfo) <> tn)) >= 1

containsPolicyBidNFT :: Value -> BuiltinByteString -> Bool
containsPolicyBidNFT v tn = valueOf v (policyBid contractInfo) (TokenName ((prefixUnsigsBid contractInfo) <> tn)) >= 1
...
init = endpoint @"init" @() $ \() -> do
    let tx = mustPayToTheScript StartBid (Value.singleton (policyBid contractInfo) (TokenName "UnsigsBid0") 1 <> Value.singleton (policyBid contractInfo) (TokenName "UnsigsBid1") 1 <> Value.singleton (policyBid contractInfo) (TokenName "UnsigsBid2") 1) 
    void $ submitTxConstraints tradeInstance tx


-- this endpoint is just for the simulator to check if a bad actor could buy two Unsigsz in a single transaction
...
troll = endpoint @"troll" @TradeParams $ \(TradeParams{..}) -> do
    ...
    let offerUtxo0 = [ (oref, txOut, getTradeDatum txOut tx, _ciTxOutValue txOut) | (oref,(txOut, tx)) <- Map.toList utxos, case getTradeDatum txOut tx of (Offer details) -> "0" == nftId details && containsUnsigsNFT (_ciTxOutValue txOut) "0"; _ -> False]
    let offerUtxo1 = [ (oref, txOut, getTradeDatum txOut tx, _ciTxOutValue txOut) | (oref,(txOut, tx)) <- Map.toList utxos, case getTradeDatum txOut tx of (Offer details) -> "1" == nftId details && containsUnsigsNFT (_ciTxOutValue txOut) "1"; _ -> False]
    ...
    let tx = collectFromScript offerUtxo0Map Buy <> 
                ...
                mustPayToPubKey (pkh) (Value.singleton (policyUnsigs contractInfo) (TokenName ("Unsigs0")) 1) <>
                mustPayToPubKey (pkh) (Value.singleton (policyUnsigs contractInfo) (TokenName ("Unsigs1")) 1)
...
cancelBidAndBuy = endpoint @"cancelBidAndBuy" @TradeParams $ \(TradeParams{..}) -> do
    ...
    utxo <- utxosAt tradeAddress
    pkh <- Contract.ownPubKeyHash
    let bidUtxo = [ (oref, txOut, getTradeDatum txOut tx, _ciTxOutValue txOut) | (oref,(txOut, tx)) <- Map.toList utxos, case getTradeDatum txOut tx of (Bid details) -> id == nftId details && containsPolicyBidNFT (_ciTxOutValue txOut) id; _ -> False]
    case bidUtxo of
        [(bidoref, bido, Bid biddetails, bidvalue)] -> do
            let offerUtxo = [ (oref, txOut, getTradeDatum txOut tx, _ciTxOutValue txOut) | (oref,(txOut, tx)) <- Map.toList utxos, case getTradeDatum txOut tx of (Offer details) -> id == nftId details && containsUnsigsNFT (_ciTxOutValue txOut) id; _ -> False]
            case offerUtxo of
                [(offeroref, offero, Offer offerdetails, offervalue)] -> do
                    ...
                    if lovelaceAmount >= 400000000 then do
                        ...
                        let tx = collectFromScript bidUtxoMap Cancel <> 
                                ...
                                mustPayToPubKey (tradeOwner offerdetails) (Ada.lovelaceValueOf (lovelaceAmount - amount1 - amount2 - amount3)) <>
                                mustPayToPubKey (pkh) (Value.singleton (policyUnsigs contractInfo) (TokenName ("Unsigs" <> id)) 1) <>
                                mustPayToTheScript StartBid (Value.singleton (policyBid contractInfo) (TokenName ("UnsigsBid" <> id)) 1)
                        void $ submitTxConstraintsSpending tradeInstance utxo tx
                    else do
                    ...
                    if lovelaceAmount >= 400000000 then do
                        let (amount1, amount2, amount3) = (lovelacePercentage lovelaceAmount (owner1Fee2),lovelacePercentage lovelaceAmount (owner2Fee1),lovelacePercentage lovelaceAmount (extraRecipient contractInfo))
                            ...
                            tx = collectFromScript bidUtxoMap Sell <> 
                                ...
                                mustPayToPubKey (tradeOwner biddetails) (Value.singleton (policyUnsigs contractInfo) (TokenName ("Unsigs" <> id)) 1) <>
                                mustPayToTheScript StartBid (Value.singleton (policyBid contractInfo) (TokenName ("UnsigsBid" <> id)) 1)
                        void $ submitTxConstraintsSpending tradeInstance utxo tx
                    else do
                        let amount1 = lovelacePercentage lovelaceAmount (owner1Fee1)
...
bid = endpoint @"bid" @TradeParams $ \(TradeParams{..}) -> do
    ...
    utxo <- utxosAt tradeAddress
    pkh <- Contract.ownPubKeyHash
    let bidUtxo = [ (oref, txOut, getTradeDatum txOut tx, _ciTxOutValue txOut) | (oref, (txOut, tx)) <- Map.toList utxos, case getTradeDatum txOut tx of (StartBid) -> containsPolicyBidNFT (_ciTxOutValue txOut) id; (Bid details) -> nftId details == id && containsPolicyBidNFT (_ciTxOutValue txOut) id; _ -> False]
    let bidDatum = Bid TradeDetails {nftId = id, requestedAmount = 1, tradeOwner = pkh}
    case bidUtxo of
            ...
            if amount < minPrice contractInfo then traceError "amount too small" else if policyBidLength value > 1 then do
                let utxoMap = Map.fromList [(oref,o)]
                    tx = collectFromScript utxoMap BidHigher <> 
                        mustPayToTheScript bidDatum (Ada.lovelaceValueOf (amount) <> Value.singleton (policyBid contractInfo) (TokenName ("UnsigsBid" <> id)) 1) <>
                        mustPayToTheScript StartBid (policyBidRemaining value (TokenName ("UnsigsBid" <> id)))
                void $ submitTxConstraintsSpending tradeInstance utxo tx
            else do
                let utxoMap = Map.fromList [(oref,o)]
                    tx = collectFromScript utxoMap BidHigher <> 
...
sell = endpoint @"sell" @TradeParams $ \(TradeParams{..}) -> do
    ...
    utxo <- utxosAt tradeAddress
    pkh <- Contract.ownPubKeyHash
    logInfo @Haskell.String $ "owner1: " <> Haskell.show (owner1 contractInfo)
    let bidUtxo = [ (oref, txOut, getTradeDatum txOut tx, _ciTxOutValue txOut) | (oref,(txOut, tx)) <- Map.toList utxos, case getTradeDatum txOut tx of (Bid details) -> id == nftId details && containsPolicyBidNFT (_ciTxOutValue txOut) id; _ -> False]
    case bidUtxo of
        [(oref, o, Bid details, value)] -> do
            let (owner1PubKeyHash, owner1Fee1, owner1Fee2) = owner1 contractInfo
            ...
            if lovelaceAmount >= 400000000 then do
                let (amount1, amount2, amount3) = (lovelacePercentage lovelaceAmount (owner1Fee2),lovelacePercentage lovelaceAmount (owner2Fee1),lovelacePercentage lovelaceAmount (extraRecipient contractInfo))
                    ...
                    tx = collectFromScript utxoMap Sell <> 
                        ...
                        mustPayToPubKey ( walletPubKeyHash (Emulator.knownWallet 3)) (Ada.lovelaceValueOf amount3) <> -- arbitary address
                        mustPayToPubKey (pkh) (Ada.lovelaceValueOf (lovelaceAmount - amount1 - amount2 - amount3)) <>
                        mustPayToPubKey (tradeOwner details) (Value.singleton (policyUnsigs contractInfo) (TokenName ("Unsigs" <> id)) 1) <>
                        mustPayToTheScript StartBid (Value.singleton (policyBid contractInfo) (TokenName ("UnsigsBid" <> id)) 1)
                void $ submitTxConstraintsSpending tradeInstance utxo tx
            else do
                let amount1 = lovelacePercentage lovelaceAmount (owner1Fee1)
...
offer = endpoint @"offer" @TradeParams $ \(TradeParams{..}) -> do
    if amount < minPrice contractInfo then traceError "amount too small" else do
        pkh <- Contract.ownPubKeyHash
        let tradeDatum = Offer TradeDetails {nftId = id, requestedAmount = amount, tradeOwner = pkh}
            tx = mustPayToTheScript tradeDatum (Value.singleton (policyUnsigs contractInfo) (TokenName ("Unsigs" <> id)) 1)
        void $ submitTxConstraints tradeInstance tx
...
buy = endpoint @"buy" @TradeParams $ \(TradeParams{..}) -> do
    ...
    case offerUtxo of
        [(oref, o, Offer details, value)] -> do
            ...
            if lovelaceAmount >= 400_000_000 then do
                let (amount1, amount2, amount3) = (lovelacePercentage lovelaceAmount (owner1Fee2),lovelacePercentage lovelaceAmount (owner2Fee1),lovelacePercentage lovelaceAmount (extraRecipient contractInfo))
                let utxoMap = Map.fromList [(oref,o)]
...
--Only for Playground 
unsigs0 = KnownCurrency (ValidatorHash "f") "Token" (TokenName "Unsigs0" :| [])
unsigs1 = KnownCurrency (ValidatorHash "f") "Token" (TokenName "Unsigs1" :| [])
unsigs2 = KnownCurrency (ValidatorHash "f") "Token" (TokenName "Unsigs2" :| [])

unsigsBid0 = KnownCurrency (ValidatorHash "f") "Token" (TokenName "UnsigsBid0" :| [])
unsigsBid1 = KnownCurrency (ValidatorHash "f") "Token" (TokenName "UnsigsBid1" :| [])
unsigsBid2 = KnownCurrency (ValidatorHash "f") "Token" (TokenName "UnsigsBid2" :| [])
